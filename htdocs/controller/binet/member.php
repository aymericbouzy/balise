<?php
  ...
  function check_admin() {
    ...
    $terms = select_terms(array("student" => $_GET["member"], "binet" => binet, "term" => term, "rights" => editing_rights));
    header_if(is_empty($terms), 404);
    header_if(binet == KES_ID && $_GET["member"] == connected_student(), 401);
    $GLOBALS["admin"]["id"] = $_GET["member"];
  }

  function check_viewer() {
    header_if(!validate_input(array("member")), 400);
    $terms = select_terms(array("student" => $_GET["member"], "binet" => binet, "term" => term, "rights" => viewing_rights));
    header_if(is_empty($terms), 404);
