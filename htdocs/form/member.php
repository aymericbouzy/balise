<?php
  ...
  $form["redirect_to_if_error"] = path($origin_action, "member", "", binet_prefix($GLOBALS["binet"], $GLOBALS["term"]));
  $form["destination_path"] = path($destination_action, "member", "", binet_prefix($GLOBALS["binet"], $GLOBALS["term"]));
  ...
  function check_new_member($input) {
    if (!is_empty($input["student"][0])) {
      $terms = select_terms(array("binet" => $GLOBALS["binet"], "term" => $GLOBALS["term"], "student" => $input["student"][0]));
      if (!is_empty($terms)) {
